...
<head>
	<?php
		...
		@$period = $_GET['period'];
		if (!isset($period)) { 
			$now = Account::first(array('select' => "DISTINCT date_format(date,'%Y-%m') as date", "order" => "date DESC"));
			$period = $now->date->format('Y-m');
		}
		// THIS LINE MUST BE HERE
		echo "<script>$(document).ready(function() { $('#period option[value=".$period."]').attr('selected', true) });</script>";
		
		// GENERIC QUERIES
		$group = Account::all(array('select' => "DISTINCT groupname", "order" => "groupname ASC"));
    ?>
</head>
<body>
	<?php
			...
			<div class="span12">
				<center>
	                <div class="well taula">
                        <h4>SGE Accounting</h4>
						<br>
	                    <div class="pull-right">
	                    	<div id="monthly">
								<select name=period id="period" class='input-medium pull-right' onchange="location.href='index.php?period='+this.value">
									<?php
							...
							<div id="advanced" style="display:none">
								<form name="form" id="form" method="post" action="#" enctype="multipart/form-data" class="form-inline">
									...
									Group <select name="group" id="group" type="text" class="input-medium required">
                        ...
                        <table cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-hover taules" id="ctable">
							...
							<?php
								...
								foreach ($group as $g) 
								{
									...
									$a2 = Account::first(array('select' => "COUNT(DISTINCT owner) as owners, COUNT(*) as jobs, SUM(ru_wallclock) as cpu_time, (SUM(end_time)-SUM(submission_time)) as wall_time", 
																"conditions" => "groupname LIKE '$groupname' AND end_time > 0 AND date LIKE'$period%'"));
						    		$a3 = Account::first(array('select' => "COUNT(*) as ok_jobs", 
						    									"conditions" => "groupname LIKE '$groupname' AND exit_status = 0 AND date LIKE'$period%'"));
									...
									echo "
										<tr>
											<td class=firstcol><a href='group.php?groupname=$groupname&period=$period'>$groupname</a></td>
							                <td>$jobs</td>
							                <td>$ok</td>
							                <td>$okp</td>
							                <td>$err</td>
